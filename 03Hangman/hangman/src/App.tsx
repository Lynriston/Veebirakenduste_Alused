...
function App() {
  ...
  //string on loetelu e masiivis
  const [guessedLetters, setGuessedLetters] = useState<string[]>([])

  //filtreerib välja tähti mis ei ole arvatavas sõnas
  const inCorrecctLetters = guessedLetters.filter(
    letter => !wordToGuess.includes(letter)
  )

  //kui valede tähtede arv on 6 siis kaotad
  ...
  const addGuessedLetter = useCallback
      ((letter:string) => {
        //isLoser ja isWinner ja vasatavalt sellele annab tuelmuse 
    ...
    <div
      ...
      >
        ...
        >
          ...
          {isLoser && "Haha, Try! - Refresh to try again!"}
        </div>

        {/* kutsume esile erinevad komponentid */}
        <HangmanDrawing numberOfGuesses={inCorrecctLetters.length}/>
